...
@section('body')
    ...
    <div>
        <div>
            ...
            <form id="frmTour" name="frmTour" method="post" action="{{ route('api.camping.index') }}">
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    @if ($pages->get('nowBlock') != 1)
                        <li class="page-item"><a class="page-link" href="{{ route('api.camping.index') }}?pageNo={{ $pages->get('s_page')-1 ."&" . $pageParams }} }}">Previous</a></li>
                    @endif
                    @for ($p=$pages->get('s_page'); $p<=$pages->get('e_page'); $p++)
                        <li class="page-item {{ getSelectedText($pages->get('pageNo'), $p, 'active') }}"><a class="page-link" href="{{ route('api.camping.index') }}?pageNo={{ $p . "&" . $pageParams }}">{{ $p }}</a></li>
                    @endfor
                    @if ($pages->get('blockNum') != $pages->get('nowBlock'))
                        <li class="page-item"><a class="page-link" href="{{ route('api.camping.index') }}?pageNo={{ $pages->get('e_page')+1 . "&" . $pageParams }}">Next</a></li>
                    @endif
                </ul>
            </nav>
        </div>
...
@section('add_js')
    <script>
        $("#list li dt").on("click", function () {
            ...
            $("#frmTourDetail")
                .attr('method', 'post')
                .attr('action', "{{ route('api.camping.result') }}")
                .submit();
        })
    </script>
@stop